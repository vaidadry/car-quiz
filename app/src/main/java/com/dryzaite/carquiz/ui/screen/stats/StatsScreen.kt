...
fun StatsScreen(stats: PersistedStats) {
    Column(
        ...
        StatsGroupCard(
            ...
            topTitle =  stringResource(R.string.quiz_results),
            topValue = "${stats.lastQuizCorrect}/${stats.lastQuizTotal}",
            topSubtitle = stringResource(if (stats.lastQuizTotal == 0) R.string.quiz_no_games else R.string.great_job),
            topPercent = "${stats.lastQuizAccuracy}%",
            bottomTitle = stringResource(R.string.flashcards_label),
            bottomValue = "${stats.lastFlashcardsGuessed}/${stats.lastFlashcardsTotal}",
            bottomSubtitle = stringResource(R.string.flashcards_guessed),
            bottomPercent = "${stats.lastFlashcardsAccuracy}%"
        )

        Text(
            text = stringResource(R.string.results_all_time_best),
            style = MaterialTheme.typography.headlineMedium,
            fontWeight = FontWeight.ExtraBold,
            color = AppTextPrimary
        )
        ...
        StatsGroupCard(
            ...
            topTitle = stringResource(R.string.results_top_score),
            topValue = "${stats.bestQuizCorrect}/${stats.bestQuizTotal}",
            topSubtitle = stringResource(if (stats.bestQuizTotal == 0) R.string.quiz_no_games else R.string.quiz_master),
            topPercent = "${stats.bestQuizAccuracy}%",
            bottomTitle = stringResource(R.string.results_max_guessed),
            bottomValue = "${stats.bestFlashcardsGuessed}/${stats.bestFlashcardsTotal}",
            bottomSubtitle = stringResource(R.string.results_in_one_go),
